...
Usage: ${SCRIPT_NAME} [<options>] <file name>
       Create a P7M container for a given file
    ...
    Arguments:
    file name : file to pack in a signed P7M container
                the output has the extension .p7m added to the file name
...
if [ "${key}" = "" ]
then
    echo "${SCRIPT_NAME}: key file not set"
    exit 1
fi
#
if [ "${cert}" = "" ]
...
if [ -r ${file}.p7m ]
then
    echo "${SCRIPT_NAME}: ${file}.p7m already exist"
    exit 1
fi
#
###############################################################################
...
if [ "${keypass}" != "" ]
then
    openssl cms -sign -cades -binary -signer ${cert} -inkey ${key} -passin pass:${keypass} -nodetach -in ${file} -md sha512 -outform der -out ${file}.p7m
    exit $?
else
    openssl cms -sign -cades -binary -signer ${cert} -inkey ${key} -nodetach -in ${file} -md sha512 -outform der -out ${file}.p7m
    exit $?
fi
